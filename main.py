...
def build_pair(data_train, data_agg, agg_ratio, oversample=1, shuffle=False):
    ...
    for data in data_train:
        ...
        for rel_pick in rel_idx:
            for _ in range(oversample):
                ...
                data_pair.append({
                    'rel': {'query': data['query'][rel_pick], 'docs': data['docs'][rel_pick]},
                    'urel': {'query': data['query'][urel_pick], 'docs': data['docs'][urel_pick]}
                })
    for data in data_agg:
        ...
        for i in range(int(dsize * agg_ratio)):
            ...
            urel_dict['query'] = data['query'][i]
            data_pair.append({
                'rel': {'query': data['query'][i], 'docs': data['docs'][i]},
                'urel': urel_dict
            })
...
def train(min_map,
    ...
    for split in splits:
        ...
        #bert_params = {'params': [v for k, v in params if k.startswith('bert.')], 'lr': 2e-5}
        #optimizer = torch.optim.Adam([non_bert_params, bert_params], lr=1e-3)
            ...
            #for batch in pgbar(range(len(data_pair) // batch_size), pre='[ SPLIT %d - TRAIN %d ]' % (split, epoch)):
                ...
                if (batch + 1) % (batch_num // 10) == 0:
                    ...
                    if max_map < metric['map']:
                        ...
                        save_path = 'save/%s/%d_%.6f.pth' % (model_name, split, metric['map'])
                    ...
                    model.train()
                    print('E%d: loss %.4f' % (epoch, losses / (batch_num //  10)))
                    losses = 0.

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, choices=RANKER.keys(), default='cedr_knrm')
    parser.add_argument('--split', type=int, nargs='*', default=[1,2,3,4,5])
    parser.add_argument('--epoch', type=int, default=1)
    parser.add_argument('--agg_ratio', type=float, default=0.)
    parser.add_argument('--oversample', type=int, default=5)
    parser.add_argument('--batch_size', type=int, default=1)
    parser.add_argument('--batch_size_test', type=int, default=31)
    parser.add_argument('--min_map', type=float, default=0.32)
    parser.add_argument('--load_path', type=str, default=None)
    parser.add_argument('--agg_path', type=str, default='data/gpt2-large-docs.pkl')
